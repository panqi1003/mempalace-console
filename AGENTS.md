...
MemPalace 本地记忆宫殿的**只读**可视化管理台：FastAPI 后端 + 原生前端（无构建链），15 个 `/api/*` 只读端点，9 个视图（总览/结构/搜索/KG/导航图/Diary/协调域/运维/记忆体检）。
...
- **页面提示 "palace unreachable"**：mempalace CLI 找不到或宫殿路径不对 → 检查前置检查两条，必要时写 `mempalace_exe` / `palace`。
