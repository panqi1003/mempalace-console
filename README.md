...
| 语义搜索 | 向量检索、similarity 排序、wing/room/source 归属、命中高亮 |
...
# 1) 安装（任选平台）
python -m venv .venv
.venv/bin/pip install fastapi uvicorn pytest httpx        # macOS/Linux
...
前置要求：本机已安装 mempalace CLI（在 PATH 中，或通过配置文件指定路径），并已 mine 过内容。
...
**中英文切换**：界面支持中文 / English。检测顺序：浏览器语言（或手动选择，保存在本地）→ 浏览器语言含中文（zh*）则默认中文，否则默认英文。右上角「中文 / EN」随时手动切换，切换后立即生效且被记住。后端用户可见的少量错误文案随请求头 `Accept-Language` 切换。
